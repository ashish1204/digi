...
<script type="text/javascript">
function reserveDeviceUS(device_id,deviceName,vendor,time){
	//var time = document.getElementById("time"_device_id).value;
	//alert(time); 
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/reserveDeviceUS?device_id="+device_id+"&deviceName="+deviceName+"&vendor="+vendor+"&time="+time,
		...
		success : function(data) {
			...
			for(var i=0; i<rowLength; i+=1){
			  ...
			  var cell = row.cells[10].innerHTML; 
			   if(cell.includes("reserveDeviceUS")){
				var cell= cell.split("reserveDeviceUS(")[1];
				 cell=cell.split(")")[0]; 
				 var m = cell.match(/'(.*?)'/); 
				 if(m[1]==device_id){
					row.cells[10].innerHTML="<td align='center'><input type='button' class='btn btn-primary'  onclick=\"return releaseDeviceUS( \'"+data.deviceId+"\',\'"+data.deviceName+"\',\'"+data.vendor+"\')\" style ='background-color: #FDB45C' value='Release'>"; 
...
<script type="text/javascript">
function releaseDeviceUS(device_id,deviceName,vendor){
	//var time = document.getElementById("time"_device_id).value;
	//alert("comingggg"); //var device_id='21'; var deviceName='asusNexus'; var vendor='seetest';
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/releaseDeviceUS?device_id="+device_id+"&deviceName="+deviceName+"&vendor="+vendor,
		contentType : "application/text; charset=utf-8",
		dataType : "json",
		success : function(data) {
			alert(data.message);
			...
			for(var i=0; i<rowLength; i+=1){
			  var row = table.rows[i];  
			  var cell = row.cells[10].innerHTML; 
			   if(cell.includes("releaseDeviceUS")){
				var cell= cell.split("releaseDeviceUS(")[1];
				 cell=cell.split(")")[0]; 
				 var m = cell.match(/'(.*?)'/); 
				 if(m[1]==device_id){
					row.cells[10].innerHTML=" <td align='center'><input type='button' class='btn btn-primary'  onclick=\"return reserveDeviceUS( \'"+device_id+"\',\'"+deviceName+"\',\'"+vendor+"\',\'"+time+"\')\" style ='background-color: #FDB45C' value='Reserve'>"; 
					...
					<table id="table" align="left"
						...
						<tbody>
							...
							<c:forEach items="${devList1}" var="deviceList1">
								<c:if
									test="${ deviceList1.vendor == 'seetest' || deviceList1.vendor == 'perfecto_partner' }">
									<c:if test="${deviceList1.reservation == 'false'}">
										...
										<tr>
											...
											<td align="left"><c:out
													value="${deviceList1.resolution}" /></td>
											...
											<%--   <form name="time" onclick="reserveDeviceUS" method="post">   --%>
											<td><input type="number" min="1" max="3" value="0"
												name="time" onchange="geTime(this.value)" /></td>

											<c:if test="${deviceList1.status == 'Available'}">
												<td align="center"><input type="button"
													onclick="return reserveDeviceUS('${deviceList1.device_id}','${deviceList1.name}','${deviceList1.vendor}',time)"
													class='btn btn-primary' style="background-color: #50CF4D"
													value="Reserve"> <%--  <input type="hidden" name="deviceid" value="${deviceList1.device_id}">
                                                                                  <input type="hidden" name="deviceName" value="${deviceList1.name}">
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}">     --%>
												</td>
											</c:if>
										...
										<tr>
											...
											<c:if test="${deviceList1.reservation == 'true'}">
												<td align="center"><input type="button"
													...
													onclick="return releaseDeviceUS('${deviceList1.device_id}','${deviceList1.name}','${deviceList1.vendor}')"
													style="background-color: #FDB45C" value="Release">
													<%-- <input type="hidden" name="deviceid" value="${deviceList1.device_id}">
                                                                                  <input type="hidden" name="deviceName" value="${deviceList1.name}">
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}">   --%>
												</td>
											...
											<td align="center"><input type="checkbox"
												value="${deviceList1.device_id}"
												id="${deviceList1.device_id}"
												onchange="getconfirm(this.id);" /></td>
											<input type="hidden" name="deviceid"
												value="${deviceList1.device_id}">
											<input type="hidden" name="deviceName"
												value="${deviceList1.name}">
