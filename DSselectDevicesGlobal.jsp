...
<script type="text/javascript">
function reserveDeviceGlobal(device_id,deviceName,vendor,time){
	//var time = document.getElementById("time"_device_id).value;
	//alert(time); 
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/reserveDeviceGlobal?device_id="+device_id+"&deviceName="+deviceName+"&vendor="+vendor+"&time="+time,
		...
		success : function(data) {
			...
			for(var i=0; i<rowLength; i+=1){
			  ...
			  var cell = row.cells[10].innerHTML; 
			   if(cell.includes("reserveDeviceGlobal")){
				var cell= cell.split("reserveDeviceGlobal(")[1];
				 cell=cell.split(")")[0]; 
				 var m = cell.match(/'(.*?)'/); 
				 if(m[1]==device_id){
					row.cells[10].innerHTML="<td align='center'><input type='button' class='btn btn-primary'  onclick=\"return releaseDeviceGlobal( \'"+data.deviceId+"\',\'"+data.deviceName+"\',\'"+data.vendor+"\')\" style ='background-color: #FDB45C' value='Release'>"; 
					 row.cells[7].innerHTML="In Use"; 
					 row.cells[9].innerHTML= "End Time:"+data.endTime;
					 row.cells[11].innerHTML=  "  <td align='center'><input type='checkbox'  id='"+data.deviceId+"' value='"+data.deviceId+"' onchange='getconfirm(this.id)'/>";
				 }
			  } 
...
<script type="text/javascript">
function releaseDeviceGlobal(device_id,deviceName,vendor){
	//var time = document.getElementById("time"_device_id).value;
	//alert("comingggg"); //var device_id='21'; var deviceName='asusNexus'; var vendor='seetest';
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/releaseDeviceGlobal?device_id="+device_id+"&deviceName="+deviceName+"&vendor="+vendor,
		contentType : "application/text; charset=utf-8",
		dataType : "json",
		success : function(data) {
			alert(data.message);
			...
			for(var i=0; i<rowLength; i+=1){
			  var row = table.rows[i];  
			  var cell = row.cells[10].innerHTML; 
			   if(cell.includes("releaseDeviceGlobal")){
				var cell= cell.split("releaseDeviceGlobal(")[1];
				 cell=cell.split(")")[0]; 
				 var m = cell.match(/'(.*?)'/); 
				 if(m[1]==device_id){
					row.cells[10].innerHTML=" <td align='center'><input type='button' class='btn btn-primary'  onclick=\"return reserveDeviceGlobal( \'"+device_id+"\',\'"+deviceName+"\',\'"+vendor+"\',\'"+time+"\')\" style ='background-color: #FDB45C' value='Reserve'>"; 
					
					 row.cells[7].innerHTML="Available"; 
					 row.cells[9].innerHTML= "<td><input type='number'  min='1' max='3' value='0' name='time' onchange='geTime(this.value)'/>";
...
<li ><a
						href="../../mobileLab/deviceSelectionMatrix/DSHomeAustraliaPage">Australia</a></li>
				</ul></li>
				<li >
				...
				<li  class="active dropdown">
					...
					<a class="dropdown-toggle buttonlink" 
					data-toggle="dropdown"><b><button class="btn mobileLabMenu">Device Recommendation<span class="caret"></span></button></b></a>
				<ul class="dropdown-menu">
					<li ><a
	...
	<div class="marginTop50px"></div>
		...
		<div class="row col-md-12">
			...
			<div class="alert alert-success alert-dismissible" role="alert">
				...
				${message}<script>speak('${message }')</script>
			</div>
		</c:if>
					  ...
					  <c:if test="${ not empty devList1 }">
					   <!-- <div id="table-wrapper">
					    <div id="table-scroll">  -->
                     <table id="table" align="left" class="table datatable table-hover table-striped table-bordered table-responsive"
                           ...
                           <c:forEach items="${devList1}" var="deviceList1"> 
                             <c:if test="${deviceList1.vendor == 'seetest' || deviceList1.vendor == 'perfecto_partner'  || deviceList1.vendor == 'other'}"> 
                                 ...
                                 <c:if test="${deviceList1.reservation == 'false'}" >   
                                                ...
                                                <tr>
                                                    ...
                                                    <%--    <td align="center"><input type="checkbox"  value="${deviceList1.device_id}" id="${deviceList1.device_id}" onchange="getButton(this.id);"/></td>
                                                                                  ...
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}"> --%>
                                                                                                ...
                                                                                                <td align="left"><label > <c:out value="${deviceList1.name}" /></label></td>
                                                                                                <td align="left"><c:out value="${deviceList1.os}" /></td>
                                                                                                <td align="left"><c:out  value="${deviceList1.version}" /></td>
                                                                                                <td align="left" style= "word-wrap:break-word;"><label > <c:out value="${deviceList1.device_id}" /></label></td>
                                                                                                <td align="left"><c:out value="${deviceList1.agentlocation}" /></td>
														                                        <td align="left"><c:out value="${deviceList1.devicecategory}" /></td>
														                                        <td align="left"><c:out value="${deviceList1.resolution}" /></td>
                                                                                                <td align="left"><c:out  value="${deviceList1.status}" /></td>
                                                                                                 <c:if test="${deviceList1.vendor == 'other'}">    
                                                                ...
                                                                <td align="left">Hosted</td>
                                                                 ...
                                                                 <td align="left">Emulator</td>
                                                                                             ...
                                                                                             <%--   <form name="time" onclick="reserveDeviceGlobal" method="post">   --%>               
                                                                                                <td><input type="number"  min="1" max="3" value="0" name="time" onchange="geTime(this.value)"/></td>
                                                                                              
                                                                                                <c:if test="${deviceList1.status == 'Available'}">    
                                                                     <td align="center"><input type="button" onclick="return reserveDeviceGlobal('${deviceList1.device_id}','${deviceList1.name}','${deviceList1.vendor}',time)" class='btn btn-primary' style ="background-color: #50CF4D" value="Reserve"> 
                                                                                  <%--  <input type="hidden" name="deviceid" value="${deviceList1.device_id}">
                                                                                  <input type="hidden" name="deviceName" value="${deviceList1.name}">
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}">     --%>  
                                                                     </td>
                                                                ...
                                                                <%-- </form>  --%>
                                                                 ...
                                                                 <c:if test="${deviceList1.vendor == 'other'}">    
                                                                     <td align="center"><a href="" class="btn btn-info">Unavailable</a></td>
                                                                 </c:if>
                                                       ...
                                                       <c:if test="${deviceList1.reservation == 'true'}" >       
                                                                           ...
                                                                           <tr>
                                                                                <%--   <td align="center"><input type="checkbox"  value="${deviceList1.device_id}" id="${deviceList1.device_id}" onchange="getButton(this.id);"/></td>
                                                                                  ...
                                                                                  <td align="left"><c:out value="${deviceList1.os}" /></td>
                                                                                  <td align="left"><c:out       value="${deviceList1.version}" /></td>
                                                                                   <td align="left"><label > <c:out value="${deviceList1.device_id}" /></label></td>
                                                                                   <td align="left"><c:out value="${deviceList1.agentlocation}" /></td>
														                           <td align="left"><c:out value="${deviceList1.devicecategory}" /></td>
														                           <td align="left"><c:out value="${deviceList1.resolution}" /></td>
                                                                                  <td align="left"><c:out       value="${deviceList1.status}" /></td>
                                               ...
                                               <%--  <form name="time" action="releaseDeviceGlobal" method="post">--%>
                                                                                         <td align="left">End Time:${deviceList1.reservation_time}</td> 
                                                                                  
                                                                                  
                                                                                         
                                                                                  <c:if test="${deviceList1.reservation == 'true'}">    
                                                             <td align="center"><input type="button" class="btn btn-primary" onclick="return releaseDeviceGlobal('${deviceList1.device_id}','${deviceList1.name}','${deviceList1.vendor}')" style ="background-color: #FDB45C" value="Release">
                                                              <%-- <input type="hidden" name="deviceid" value="${deviceList1.device_id}">
                                                                                  <input type="hidden" name="deviceName" value="${deviceList1.name}">
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}">   --%>
                                                                                  </td>
