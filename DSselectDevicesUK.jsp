...
<script type="text/javascript">
function reserveDeviceUK(device_id,deviceName,vendor,time){
	//var time = document.getElementById("time"_device_id).value;
	//alert(time); 
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/reserveDeviceUK?device_id="+device_id+"&deviceName="+deviceName+"&vendor="+vendor+"&time="+time,
		...
		success : function(data) {
			...
			for(var i=0; i<rowLength; i+=1){
			  ...
			  var cell = row.cells[10].innerHTML; 
			   if(cell.includes("reserveDeviceUK")){
				var cell= cell.split("reserveDeviceUK(")[1];
				 cell=cell.split(")")[0]; 
				 var m = cell.match(/'(.*?)'/); 
				 if(m[1]==device_id){
					row.cells[10].innerHTML="<td align='center'><input type='button' class='btn btn-primary'  onclick=\"return releaseDeviceUK( \'"+data.deviceId+"\',\'"+data.deviceName+"\',\'"+data.vendor+"\')\" style ='background-color: #FDB45C' value='Release'>"; 
...
<script type="text/javascript">
function releaseDeviceUK(device_id,deviceName,vendor){
	//var time = document.getElementById("time"_device_id).value;
	//alert("comingggg"); //var device_id='21'; var deviceName='asusNexus'; var vendor='seetest';
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/releaseDeviceUK?device_id="+device_id+"&deviceName="+deviceName+"&vendor="+vendor,
		contentType : "application/text; charset=utf-8",
		dataType : "json",
		success : function(data) {
			alert(data.message);
			...
			for(var i=0; i<rowLength; i+=1){
			  var row = table.rows[i];  
			  var cell = row.cells[10].innerHTML; 
			   if(cell.includes("releaseDeviceUK")){
				var cell= cell.split("releaseDeviceUK(")[1];
				 cell=cell.split(")")[0]; 
				 var m = cell.match(/'(.*?)'/); 
				 if(m[1]==device_id){
					row.cells[10].innerHTML=" <td align='center'><input type='button' class='btn btn-primary'  onclick=\"return reserveDeviceUK( \'"+device_id+"\',\'"+deviceName+"\',\'"+vendor+"\',\'"+time+"\')\" style ='background-color: #FDB45C' value='Reserve'>"; 
...
<li><a
  ...
  <c:if test="${deviceList1.vendor == 'seetest' || deviceList1.vendor == 'perfecto_partner'}">
                                 <c:if test="${deviceList1.reservation == 'false'}" >   
                                                    ...
                                                    <%--    <td align="center"><input type="checkbox"  value="${deviceList1.device_id}" id="${deviceList1.device_id}" onchange="getButton(this.id);"/></td>
                                                                ...
                                                                <td align="left">Hosted</td>
                                                                 ...
                                                                 <td align="left">Emulator</td>
                                                                                             ...
                                                                                             <%--   <form name="time" onclick="reserveDeviceUK" method="post">   --%>               
                                                                                                <td><input type="number"  min="1" max="3" value="0" name="time" onchange="geTime(this.value)"/></td>
                                                                                              
                                                                                                <c:if test="${deviceList1.status == 'Available'}">    
                                                                     <td align="center"><input type="button" onclick="return reserveDeviceUK('${deviceList1.device_id}','${deviceList1.name}','${deviceList1.vendor}',time)" class='btn btn-primary' style ="background-color: #50CF4D" value="Reserve"> 
                                                                                  <%--  <input type="hidden" name="deviceid" value="${deviceList1.device_id}">
                                                                                  <input type="hidden" name="deviceName" value="${deviceList1.name}">
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}">     --%>  
                                                                     </td>
                                               ...
                                               <%--  <form name="time" action="releaseDeviceUK" method="post">--%>
                                                                                         <td align="left">End Time:${deviceList1.reservation_time}</td> 
                                                                                  
                                                                                  
                                                                                         
                                                                                  <c:if test="${deviceList1.reservation == 'true'}">    
                                                             <td align="center"><input type="button" class="btn btn-primary" onclick="return releaseDeviceUK('${deviceList1.device_id}','${deviceList1.name}','${deviceList1.vendor}')" style ="background-color: #FDB45C" value="Release">
                                                              <%-- <input type="hidden" name="deviceid" value="${deviceList1.device_id}">
                                                                                  <input type="hidden" name="deviceName" value="${deviceList1.name}">
                                                                                  <input type="hidden" name="vendor" value="${deviceList1.vendor}">   --%>
                                                                                  </td>
