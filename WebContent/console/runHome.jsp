...
function commandExecute () {
	...
	if('webkitSpeechRecognition' in window){
		...
		if(interimTranscripts.toLowerCase().includes("trigger")){
			var index=runs.indexOf(interimTranscripts.toLowerCase().split("trigger")[1].trim());
			if(index>-1){
				window.location.href=a+'/console/triggerRun?runName='+finalRuns[index].trim()+'&toolName=${toolName}';
			}
		}else if(interimTranscripts.toLowerCase().includes("delete")){
			var index=runs.indexOf(interimTranscripts.toLowerCase().split("delete")[1].trim());
			if(index>-1){
				
				var fullRun = document
				.getElementById("runsTable").rows[index+1].cells[0].innerHTML;

			var runid = fullRun.split("runs=")[1].split("\">")[0];
				
				
				window.location.href=a+'/console/deleteRun?runid='+runid+'&toolName=${toolName}';
			}
		}
		else if(interimTranscripts.toLowerCase().includes("download")){
			var index=runs.indexOf(interimTranscripts.toLowerCase().split("download")[1].trim().split("report")[0].trim());
			if(index>-1){
		
				var runHome = document
				.getElementById("runsTable").rows[index+1].cells[3].innerHTML;
				
				window.location.href=a+'/console/getReport?runHome='+runHome;
			}
		}
		};
												...
												for (key in result) {
													...
													for (var index = 1; index<rowCount && rowCount>0; index++) {
														...
														if (run == key) {
															...
															if (result[key] == "FAILURE"
																|| result[key] == "SUCCESS" || result[key] == "Pending" || result[key] == "Progress"
																	|| result[key] == "Jenkins Offline") {
																...
																document
																		.getElementById("runsTable").rows[index].cells[4].innerHTML = "-";
															}

															if (result[key] == "FAILURE"
																	|| result[key] == "SUCCESS" || result[key] == "Pending" || result[key] == "Progress"
																		|| result[key] == "Jenkins Offline") {
																var insideText = "<a href=\"./getReport?runHome="
																		+ runHome
																...
																document
																		.getElementById("runsTable").rows[index].cells[5].innerHTML = "-";
															}

															if (result[key] == "FAILURE"
																|| result[key] == "SUCCESS" || result[key] == "Pending" || result[key] == "Progress"
																	|| result[key] == "Jenkins Offline") {
																var insideText = "<a href=\"./deleteRun?runid="
																		+ runid
																...
																document
																		.getElementById("runsTable").rows[index].cells[6].innerHTML = "-";
															}
			...
			<div class="col-sm-8 col-sm-offset-2">
				...
				<c:if test="${not empty successRun}">
					<div class="alert alert-success alert-dismissible" role="alert">
						...
						<script>speak('${successRun }')</script>
					</div>
					<br>
				</c:if>
				...
				<c:if test="${not empty triggerMessage}">
					<div class="alert alert-success alert-dismissible" role="alert">
						...
						<script>speak('${triggerMessage }')</script>
					</div>
					<br>
				</c:if>
				...
				<c:if test="${not empty deleteMessage}">
					<div class="alert alert-success alert-dismissible" role="alert">
						...
						<script>speak('${deleteMessage }')</script>
					</div>
					<br>
				</c:if>
				...
				<c:if test="${not empty errorMessage}">
					<div class="alert alert-danger alert-dismissible" role="alert">
						...
						<script>speak('${errorMessage}')</script>
					</div>
					<br>
				</c:if>
			</div>
