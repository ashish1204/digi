...
function requestApproval(status,reservationid){
	var comment=document.getElementById("comment_"+reservationid).value;
	$
	.ajax({
		type : "GET",
		url : "${pageContext.request.contextPath}/integratedQALabs/mobileLab/deviceSelectionMatrix/requestApproval",
		data:{"status":status,
...
var table=document.getElementById('requestTable');
			...
			for(var i=0; i<rowLength; i+=1){
				  ...
				  var cell = row.cells[7].innerHTML;
						  if(cell.includes("requestApproval(")){
...
<!-- START WIDGETS -->                    
    ...
    <div class="top-bar">
			<a href=""></a>
			<div class="font24px">
				<label>Device Requests</label>
			</div>

		</div>                
  	<c:if test="${empty deviceRequests }">
	...
	<c:if test="${ not empty deviceRequests }">
		...
		<thead>
			...
			<tbody>
			<c:forEach items="${deviceRequests }" var="request">
				<tr>
					...
					<td><textarea rows="2" cols="5"
							id='comment_${request.reservationid }' class="form-control" rows="10"></textarea></td>
					<td>
						<button
							onclick="return requestApproval('Approved','${request.reservationid }')" class="btn btn-info btn-block">Approve</button>
					</td>
					<td>
						<button
							onclick="return requestApproval('Declined','${request.reservationid }')" class="btn btn-danger btn-block">Reject</button>
					</td>
