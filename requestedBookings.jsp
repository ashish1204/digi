...
<title>All Requested Booking</title>
		...
		<div class="row">
			...
			<div class="col-md-9">
				...
				<div class="top-bar">
					...
					<div class="font24px">
						<label>Requested Booking</label>
					</div>

				</div>
				<c:if test="${ not empty requestedBookings }">
					<c:if test="${sessionScope.rolename == 'ADMIN'}">
						...
						<table align="left" class="table table-striped table-hover "
							...
							<tbody>
								<c:forEach items="${requestedBookings}" var="requestedBookings">
									<tr>
										<td>${requestedBookings.devicename}</td>
										<td>${requestedBookings.os}</td>
										<td>${requestedBookings.version}</td>
										<td>${requestedBookings.resolution}</td>
										<td>${requestedBookings.vendor}</td>
										<td>${requestedBookings.createdby}</td>
										<td>${requestedBookings.starttime}</td>
										<td>${requestedBookings.endtime}</td>
										<td>${requestedBookings.approvalStatus}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
