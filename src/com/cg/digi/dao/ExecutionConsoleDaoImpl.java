...
public class ExecutionConsoleDaoImpl implements IExecutionConsoleDao {
	...
	public Run addRun(Run run) {
		...
		Object[] params = new Object[] { run.getRunName(), run.getRunRemarks(),
				run.getBuildid(), run.getToolid(), run.getScheduledtime(),
				run.getScheduledStatus(), run.getToollanguageid(),
				run.getUserid(), "Y",
		...
		try {
			...
			run = getRun(runid);
			 DigiLoggerUtils.log(
						"Run Details in addRun(Run run) Dao :: "+runid+" runid -->>" + run,
						LEVEL.info);
			}else{
				 run = new Run();
				 DigiLoggerUtils.log(
							"Run Details in addRun(Run run) Dao :At Error Null: " + run,
							LEVEL.info);
				return run;
			}
		} catch (DataAccessException e) {
	...
	public List<Run> getRuns(String projectid, String toolid) {
		String query = "SELECT r.runid,r.runname,r.runsremarks,r.scheduledStatus,r.creationTime,b.buildname,t.toolname,r.scheduledtime,r.createdby,r.creationtime,r.modifiedby,r.modifiedtime,r.status,r.runhome,r.userid from runs r,builds b, tool t WHERE userid IN (select userid from users where projectid = '"+projectid+"' and status = 'Y' )   and r.toolid ="+toolid+" and r.toolid = t.toolid and r.buildid = b.buildid and r.status = 'Y' ";
		List<Run> runs = null;
		try {
			runs = jdbcTemplate.query(query, new BeanPropertyRowMapper(Run.class));
			DigiLoggerUtils.log("Run details in Dao loginValidation() : "
					+ runs, LEVEL.trace);
		} catch (DataAccessException e) {
			DigiLoggerUtils.log("DataAccessException in getRuns() :: " + e,
					LEVEL.error);
		} catch (Exception e) {
	...
	public boolean ModifyRun(Run run) {
		...
		out=	jdbcTemplate.update(query,params);
			...
			e.printStackTrace();
			DigiLoggerUtils.log("Error in ModifyRun :: run name "+run.getRunName()+":Exception Message: "+e.getStackTrace(), LEVEL.error);
		}
		if(out>=1){
			return true;
			}
	...
	public Run getRun(int runid) {
		...
		try {
			...
			DigiLoggerUtils.log("Run details in Dao getRun() :based on latest update Runid::  "
					+ runs, LEVEL.trace);
		} catch (DataAccessException e) {
			DigiLoggerUtils.log("DataAccessException in getRuns() : " + e,
					LEVEL.error);
	...
	public boolean checkRun(String runId) {
		...
		try {
			...
			DigiLoggerUtils.log("DataAccessException in checkRun() : " + e,
					LEVEL.error);
		}catch (Exception e) {
	...
	public Run deleteRun(String runid) {
		...
		try {
			...
			DigiLoggerUtils.log("Error while delteing run :: run id "+runid+":Exception Message: "+e.getStackTrace(), LEVEL.error);
		}
		if(out>=1){
	...
	public boolean modifyRunStatus(String runName, String buildStatus) {
		int out = 0;
		try{
	out=	jdbcTemplate.update("UPDATE runs SET scheduledStatus='"+buildStatus+"' WHERE runname='"+runName+"' AND status='Y'");
		}catch (DataAccessException e) {
			DigiLoggerUtils.log("Error while updating run status:: run name :"+runName+" Run Status: "+buildStatus+":Exception Message: "+e.getMessage(), LEVEL.error);
		}
		if(out>0){
		return true;
		}else{
