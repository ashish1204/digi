...
public class ExecutionConsoleServiceImpl implements IExecutionConsoleService {
	...
	public List<Run> getRuns(String projectid, String toolid) {
		DigiLoggerUtils.log("Run Details in Service Layer "
				+ executionConsleDao.getRuns(projectid,toolid), LEVEL.info);
	...
	@Override
	public File getFinalRunPath(String projectName, String userName,
			String toolName, String continonusDeliveryHome, String runName) {
		...
		if (!runHome.exists()) {
			...
			runHome = FileUtils
					.createFolder(runHome.getAbsolutePath(), runName);
		}
		DigiLoggerUtils.log("Run Home in Service Layer : " + runHome,
				LEVEL.info);
		return runHome;
	}
	...
	public long getTimeDifference(String time) {
		...
		try {
			...
			System.out.println("Error While calculating difference "
					+ e.getStackTrace());
	...
	public boolean ModifyRun(Run run) {
		/*if(executionConsleDao.checkRun(run.getRunName())){
			if( executionConsleDao.ModifyRun(run)){
			return true;
			}else{
				return false;
		}
